...
import static uk.gov.hmrc.jenkinsjobs.domain.publisher.Publishers.buildSlugJobsTriggerPublisher
    ...
    void 'test XML output'() {
        ...
        JobBuilder jobBuilder = new JobBuilder('test-job', 'test-job-description', 14, 10, gitHubComScm('example/example-repo')).
                                               ...
                                               withPublishers(jUnitReportsPublisher('test-junit'),
                                                              ...
                                                              buildSlugJobsTriggerPublisher([APP: '${JOB_NAME}', APP_BUILD_NUMBER: '${BUILD_NUMBER}']),
        ...
        with(job.node) {
            ...
            publishers.'hudson.plugins.parameterizedtrigger.BuildTrigger'.configs.'hudson.plugins.parameterizedtrigger.BuildTriggerConfig' [0].projects.text() == 'build-slug-preprocess-play-microservice'
            ...
            publishers.'hudson.plugins.parameterizedtrigger.BuildTrigger'.configs.'hudson.plugins.parameterizedtrigger.BuildTriggerConfig' [0].configs.'hudson.plugins.parameterizedtrigger.PredefinedBuildParameters'.properties.text().contains('APP=${JOB_NAME}\nAPP_BUILD_NUMBER=${BUILD_NUMBER}')
            publishers.'hudson.plugins.parameterizedtrigger.BuildTrigger'.configs.'hudson.plugins.parameterizedtrigger.BuildTriggerConfig' [1].projects.text() == 'test-jobs'
            publishers.'hudson.plugins.parameterizedtrigger.BuildTrigger'.configs.'hudson.plugins.parameterizedtrigger.BuildTriggerConfig' [1].condition.text() == 'SUCCESS'
        }
    }
}
