...
class SbtMicroserviceJobBuilderSpec extends Specification {
    ...
    void 'test XML output with fun:tests'() {
        ...
        then:
        with(job.node) {
            builders.'hudson.tasks.Shell'.command.text().contains('sbt $SBT_OPTS -mem 8192 clean validate test it:test fun:test dist-tgz +publishSigned')
        }
    }
    ...
    void 'test scoverage output'() {
        ...
        then:
        with(job.node) {
            builders.'hudson.tasks.Shell'.command.text().contains('sbt $SBT_OPTS -mem 8192 clean validate coverage test coverageOff coverageReport dist-tgz +publishSigned')
    ...
    void 'test scalastyle output'() {
        ...
        then:
        with(job.node) {
            builders.'hudson.tasks.Shell'.command.text().contains('sbt $SBT_OPTS -mem 8192 clean validate scalastyle test dist-tgz +publishSigned')
    ...
    void 'test scoverage and scalastyle output'() {
        ...
        then:
        with(job.node) {
            builders.'hudson.tasks.Shell'.command.text().contains('sbt $SBT_OPTS -mem 8192 clean validate scalastyle coverage test coverageOff coverageReport dist-tgz +publishSigned')
            publishers.'org.jenkinsci.plugins.scoverage.ScoveragePublisher'.reportDir.text() == "target/scala-2.11/scoverage-report"
            publishers.'org.jenkinsci.plugins.scoverage.ScoveragePublisher'.reportFile.text() == "scoverage.xml"
            publishers.'hudson.plugins.checkstyle.CheckStylePublisher'.pluginName.text() == "[CHECKSTYLE]"
        }
